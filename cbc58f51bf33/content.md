Sometimes we'd like to be able to predict facts about the members of some population. We can use data about a sample of the population to discover relationships between variables in the population, and we can use those relationships to make predictions.

# A muffin shop problem

Suppose you run a muffin shop. You've noticed that the demand for muffins varies quite a bit from day to day, for no obvious reason. It doesn't seem to be due to the day of the week, or the weather, or anything else. You'd love to be able to predict what the demand is going to be, at least a day in advance, because you're losing money by making too few or too many muffins.
...
You can see that there's a positive correlation, in this sample of days, between the demand for muffins and the activity of the northern lights: as the activity increases, so does the demand; as the activity decreases, so does the demand. You can also see that the correlation seems to be linear.

# An equation

You calculate the equation of the line that best fits your sample data (there are mathematical techniques for this), and this is what you get:
...
You've found what's likely to be a real relationship between muffin demand and northern lights activity, and you have an equation for it.
...
This prediction is unlikely to be perfect, because of random fluctuations in muffin demand and northern lights activity. They effected the equation that you found - if you'd used a different sample of days, the random fluctuations would likely have been different, and so too would the equation you found. And they effect the quality of your predictions - even if your equation is perfect, random fluctuations mean that its predictions won't be. Nevertheless, using it to make predictions is better than what you were previously doing, which was basically just guessing.    
...
The equation you've found, between muffin demand and northern lights activity, is called a **model**. It's a model of one variable, muffin demand, in terms of another variable, northern lights activity. We call the variable being modelled (muffin demand) the **target variable** (or "dependent variable", or "response variable"). And we call the variable being used to model it (northern lights activity) the **predictor variable** (or "independent variable").

This particular model is a linear model. Linear models are very common, but there are many other kinds of models, and sophisticated techniques for finding them. The process of finding models, and using them to make predictions, is called **predictive analytics**.
...
When we find a model of a target variable in terms of some predictor variable, it can be tempting to conclude that the target variable *causes* the predictor variable. We humans have a voracious appetite for causal explanations, and if we can see even a remotely plausible causal explanation of a model, we'll jump to it.
...
As tempting as it is, we must not draw any causal conclusions from a model. Just because there's a relationship between two variables, even one that can be described with an equation, we cannot conclude anything about what's causing what. Figuring out causation is a different process from finding models, and has its own carefully worked-out techniques. If you want to find causal connections between things, you should use that other process instead.
...
Here are two real examples of finding models and using them to make predictions. They're quite famous - perhaps you've heard of them?
